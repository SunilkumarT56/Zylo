...
export function ImportConfiguration({ repo, onBack, onDeploy, loading }: ImportConfigurationProps) {
  ...
  // Use directories from repo if available, otherwise default to just current dir
  const dirs = repo.directories && repo.directories.length > 0
    ? repo.directories.map(d => d.name) // Display name alone as requested
    : ["./"]; 
                            ...
                            <div className="max-h-60 overflow-y-auto p-1">
                                ...
                                {dirs.map(dir => (
                                    <button
                                        ...
                                        className="w-full text-left px-3 py-2 text-sm text-zinc-300 hover:bg-zinc-800 hover:text-white rounded-md flex items-center gap-2.5 transition-colors group"
                                    >
                                        <Folder className="h-4 w-4 text-zinc-500 group-hover:text-blue-400 transition-colors" />
